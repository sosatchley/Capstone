...
public void serialEvent(Serial serialPort) {
    ...
    if (firstContact == false) {
        ...
        serialCount++;
        if (serialCount > 0) {
            this.wheelRotate = lerp(this.wheelRotate, map(serialInArray[0], 0, 255, radians(-720), radians(720)), .05f);
...
public void mousePressed() {
    ...
    if (mouseY > 500) {
        if (mouseButton == LEFT) {
            this.wheelSpeed = (radians(2.0f));
        } else { //1
            this.wheelSpeed = (radians(-1.0f));
        }
    } else {
        if (mouseButton == LEFT) {
...
class Wheel {
    ...
    public void turn(float angle) {
        this.speed = lerp(this.speed, angle, 0.04f);
...
class SteeringWheel {
  ...
  public void settings() {  size(600,1000); }
